...
**理由**: 単一のシグネチャで両文脈をカバーでき、ライブラリ利用者が用途ごとに型を使い分ける必要がない。Rustの主要なMustache実装でも同様のパターンが採られており、「セクション本体の生テキストを受け取る関数」という一貫したメンタルモデルで説明できる。Bのように2バリアントに分けると、利用者はどちらを使うべきか毎回判断する負担が生じる。
...
**理由**: FR-3（`~lambdas.json`に100%準拠）で既に確定している方針の具体化であり、公式spec自体がこの挙動を要求している。Bを選ぶとFR-3と直接矛盾する。
...
**理由**: 公式spec上、`{{lambda}}`と`{{{lambda}}}`/`{{&lambda}}`の書き分けはラムダ以外の変数展開と全く同じ意味を持つべきであり、タグの見た目が「ラムダの結果かどうか」でエスケープの有無を変える仕様上の区別は存在しない。BR-1.1/BR-1.2との一貫性も保たれる。
...
**理由**: クロージャの実体を比較する意味のある基準がそもそも存在しない。`Rc::ptr_eq`（B）は「同じインスタンスかどうか」は判定できるが、それが呼び出し側にとって有用な情報になる場面がなく、むしろ「一見似ているが微妙に異なる挙動」という誤解を招きやすい。常に`false`とするのが最も単純で驚きが少ない。
...
**理由**: 現在のレンダリング処理（`Renderer`）は単一スレッド同期実行であり、`Mustache`構造体自体もマルチスレッド対応を謳っていない（`with_partial_resolver`も`Box<dyn PartialResolver>`であり`Send`/`Sync`境界なし、既存の設計と一貫している）。`Arc`+`Send`/`Sync`境界は将来的な拡張余地こそあるが、今は使われない制約を利用者のクロージャに強制するだけで、YAGNIの観点からも不要。
...
**理由**: Bの「再構築（un-parse）」方式は、構文木から元のテキストへの変換ロジックそのものが新たなバグの温床になり（元のホワイトスペースやデリミタ変更表記を完全に復元できる保証が必要）、専用のテストも別途必要になる。Aはパース時点で既に手元にある文字列スライスを保持するだけで実装が単純であり、ロスレス性も保証される。メモリ増加分もテンプレートサイズに対して線形で、実用上問題にならない。
...
**理由**: 公式specでは`{{<parent}}...{{/parent}}`の本体は「差し替えるブロックの一覧」としての意味しか持たず、他のテキストは意図的に無視される仕様。Bのようにエラーにすると、コメントや説明目的でテキストを添えたい場合にも失敗してしまい、spec非準拠にもなる。
...
**理由**: `{{$block}}`は「差し替え可能でデフォルト値を持つ領域」を表すタグであり、差し替え元（`{{<parent}}`）が存在しない場合はデフォルトがそのまま採用されるのが最も自然な意味論。Bのようにエラーにする設計上の必然性がない。
...
**理由**: `mustache-optional-modules-requirements.md`のFR-6は明示的にパーシャルタグのみをスコープとしており、勝手に範囲を広げるとRequirements Analysisで確定した合意から逸脱する。公式fixtureに親タグの動的名前ケースが含まれていた場合は、Code Generation段階でのspec fixture検証時に判明するので、その時点で「対象外」として扱うか、追加の確認を行うのが適切（要件変更は正式なプロセスを経るべきという、このプロジェクトの一貫した方針）。
