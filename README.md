...
- 公式spec conformanceテスト（必須6モジュール計136ケース、オプション3モジュール計58ケース）で100%準拠を確認済み
...
ブロックの「再インデント処理」（差し替え内容の行頭インデントを、定義箇所ではなく展開箇所の位置に揃える処理）にも対応しており、公式spec conformanceテスト（`~inheritance`、27ケース）は27/27（100%）準拠です。
...
設計上の詳細な決定事項は`aidlc-docs/`配下のドキュメントを参照してください。
